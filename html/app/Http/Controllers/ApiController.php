...
class ApiController extends Controller
{
    ...
    {
        $content = '/prices.txt';
		$content2 = file_get_contents(dirname(__FILE__) . $content);
        $prices = json_decode($content2, true);
            ...
            if(strpos(Auth::user()->username, 'csgon1.com') !== false) {
                if(Auth::user()->csgo == 'true'){
                    if(Auth::user()->last_free_use + 86400 < time()){
                        DB::table('users')->where('steamid', Auth::user()->steamid)->update(array('wallet'=>DB::raw('wallet + 30'), 'last_free_use'=>time()));
                        DB::table('wallet_change')->insert(array('user'=>Auth::user()->steamid,'change'=>30,'reason'=>'Free coins '.date("Y-m-d H:i:s")));
    ...
    public function group_join()
    {
        if (Auth::check()){
			...
			$in_group = 0;
			$url = 'http://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=76EA12089450A0A6F64892830FAACB8F&steamids='.$steam;
			$file = file_get_contents($url);
			$decode = json_decode($file);
			foreach ($decode->response->players as $player) {
						$clan += $player->primaryclanid;
			} 
			if($clan = 103582791455865593) {
				$in_group = 1;
			}
