...
$(function() {
    ...
    window.roulette = ({
        ...
        updateHistory: function(number) {
			...
			for (iCrown = 0; iCrown < iiCrown; iCrown++) {
				var renderCrown = iCrown + 1;
				$('.crown-counter').html('<span> Number of rolls since last Crown:&nbsp;'+ renderCrown +'</span>');
			}
            if ($('.latest').children().length >= 25) $('.latest > div').last().remove();
        ...
        updateTimer: function() {
            if (this.timeLeft > 0) {
                $('.rolling').fadeIn(200);
                $('.rolling-inner').html('<div style="margin-bottom:7px;">' + (this.timeLeft / 100).toFixed(2).replace(".",":") + '</div>Countdown to next spin');
            } else {
                $('.rolling').fadeOut(200);
            }
        },
