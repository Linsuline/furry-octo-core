$(function() {
    ...
    window.coinflip = ({
        ...
        _generateBetFromData: function(data) {
            ...
            if(data.status === 'open') text += '<button data-hash="' + data.hash + '" style="outline: 0; border: 0; height: 25px; width: 7%; margin-top: 1%; font-size: 18px; line-height: 10px; color: #161616; background: #00ffe4; font-weight: 700; cursor: pointer; text-transform: uppercase; font-family: Titillium,sans-serif; border-radius: 5px; font-weight: 700; transition: all .3s; border: 3px solid #00ffe4;" class="btn-join" onclick="socket.emit(\'coinflip join\', \'' + data.hash + '\');">Join</button>';
